...
// Expand as needed (redirects, rewrites, etc.).
...
func (e *RouteExtractor) extractNextJSRoutesFromManifest(dir string) error {
	...
	if err != nil {
		...
		e.log.Debug("next.js routes-manifest not found", "path", manifestPath, "error", err)
		return nil
	}
	defer f.Close()
	...
	if err := json.NewDecoder(f).Decode(&manifest); err != nil {
		e.log.Debug("failed to decode next.js routes-manifest", "path", manifestPath, "error", err)
		return nil
	}
	...
	added := len(e.routes) - before
	if added > 0 {
		e.log.Debug("extracted next.js routes from manifest",
			"path", manifestPath,
			"count", added)
	}
